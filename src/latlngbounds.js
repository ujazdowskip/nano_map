...
class LatLngBounds {
	...
	constructor(a, b) {
		...
		return this._constructor(a, b);
	}

	_constructor(corner1, corner2) { // (LatLng, LatLng) or (LatLng[])
		if (!corner1) { return; }
	...
	extend(obj) {
		...
		if (obj instanceof LatLng) {
			...
			return obj ? this.extend(new LatLng(obj) || new LatLngBounds(obj)) : this;
		}
	...
	pad(bufferRatio) {
		const sw = this._southWest
		const ne = this._northEast
		const heightBuffer = abs(sw.lat - ne.lat) * bufferRatio
		const widthBuffer = abs(sw.lng - ne.lng) * bufferRatio
